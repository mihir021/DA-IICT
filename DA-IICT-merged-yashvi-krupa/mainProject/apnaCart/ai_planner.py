...
def _build_recipe_image_urls(dish: str, approved_items: list) -> list[str]:
    """Generate stable photo URLs for the dish and key approved ingredients."""
    urls = [f"https://source.unsplash.com/1200x800/?{quote_plus(dish + ',food plating')}"]
    for item in approved_items[:3]:
        name = item.get("product_name") or item.get("ingredient_name") or "grocery"
        urls.append(f"https://source.unsplash.com/900x600/?{quote_plus(name + ',ingredient')}")
    return urls


_RECIPE_SYSTEM_PROMPT = """You are a chef assistant.
Create a friendly, step-by-step recipe using ONLY approved items provided by the user.
...
Return ONLY valid JSON in this schema:
{
  ...
  "steps": [
    {"step_no": 1, "instruction": "string"}
  ],
  "tips": ["string", "string"]
}

Rules:
- Use only provided ingredients and their quantities.
- Do not invent new ingredients.
- Keep instructions practical and easy for home cooks.
- If a required ingredient is missing, adapt the method and mention that adaptation in steps.
"""


def generate_recipe_from_approved_items(dish: str, servings: int, approved_items: list) -> dict:
    """Generate a recipe guide from user-approved grocery items only."""
    if not approved_items:
        return {"error": "Please select at least one item to generate a recipe."}

    payload_items = []
    for item in approved_items:
        qty = item.get("needed_quantity") or f"{item.get('quantity', 1)} x {item.get('product_unit', '').strip()}".strip()
        payload_items.append({
            "name": item.get("product_name") or item.get("ingredient_name") or "Item",
            "quantity": qty,
        })
    ...
    try:
        ...
        return {"error": "Recipe generation returned an invalid format. Please try again."}
    except Exception as exc:
        logger.error("Recipe generation error: %s", exc)
        return {"error": f"Recipe generation failed: {str(exc)}"}

    parsed["image_urls"] = _build_recipe_image_urls(dish, approved_items)
    return parsed
